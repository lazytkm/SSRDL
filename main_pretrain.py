...
def get_args_parser():
    ...
    parser.add_argument("--dist_url", default="tcp://127.0.0.1:52112", type=str, help="""url used to set up
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, data_loader,
    ...
    for it, (images, _) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            teacher_output = teacher(crops[:2], ncrops=2)  # only the 2 global views pass through the teacher
            student_output = student(crops, ncrops=args.local_crops_number + 2)
            sample_ouput, log_var, mu = student(img, num_samples=args.num_samples)
            loss, ce_loss, kl_loss, sample_loss = dino_loss(sample_ouput, student_output, teacher_output, log_var, mu, epoch)
...
class DINOLoss(nn.Module):
    def __init__(self, out_dim, ncrops, warmup_teacher_temp, teacher_temp,
                 ...
                 center_momentum=0.9, sample_temp=0.1):
        super().__init__()
